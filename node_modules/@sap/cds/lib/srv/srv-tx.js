...
class RootContext extends EventContext { static for(_) { return _ instanceof EventContext ? _ : super.for(_,'as root') } }
class NestedContext extends EventContext { static for(_) { return _ instanceof EventContext ? _ : super.for(_) } }
...
module.exports = exports = function srv_tx (ctx,fn) { const srv = this
  ...
  // srv.tx().tx() -> idempotent no-op (questionable: ignoring all futher arguments)
  if (srv.context) return srv

  // Usage variant 1: srv.tx() -> get root tx for a service
  if (!ctx) return RootTransaction.for (srv)

  // Usage variant 2: srv.tx (fn) -> run fn in a new root tx
  if (typeof ctx === 'function') [ ctx, fn ] = [ undefined, ctx ]
  if (typeof fn === 'function') {
    const tx = RootTransaction.for (srv, ctx)
    return cds._with (tx, ()=> Promise.resolve(fn(tx)) .then (tx.commit, tx.rollback))
  }

  // Usage variant 3: srv.tx (req) -> LEGACY way of propagating tx before cds.context
  if (ctx instanceof EventContext) {
    if (ctx.tx) return NestedTransaction.for (srv, ctx)
    else return RootTransaction.for (srv, ctx)
  }

  // Usage variant 4: srv.tx ({...}) -> get a new root tx for a service
  if (srv_tx_compat_for_afc) { // REVISIT: compatibility for AFC only -> we should get rid of that
