...
const escapeSingleQuotes = val => (val.indexOf("'") >= 0 ? val.replace(/'/g, "''") : val)
...
const formatVal = (val, elementName, csnTarget, kind, func, literal) => {
  ...
  if (element.type === 'cds.LargeString' || element.type === 'cds.String') {
    val = escapeSingleQuotes(val)
  }
...
function addRefToWhereIfNecessary(where, entity, ignoreManagedBackLinks = false) {
  ...
  if (keys.filter(k => k !== 'IsActiveEntity').length !== 1) return 0
